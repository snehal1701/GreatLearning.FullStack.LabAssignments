...
public class MySecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http.authorizeHttpRequests().antMatchers("/", "/showMyLoginPage", "/registration").permitAll()
				.antMatchers("save").hasAnyRole("ROLE_ADMIN,ROLE_USER").antMatchers("update", "delete")
				.hasRole("ROLE_ADMIN").anyRequest().authenticated().and().formLogin().loginPage("/showMyLoginPage")
				.loginProcessingUrl("/authenticateTheUser").permitAll().and().logout().permitAll().and()
				.exceptionHandling().accessDeniedPage("/access-denied").and().httpBasic();
		return http.build();
	}
